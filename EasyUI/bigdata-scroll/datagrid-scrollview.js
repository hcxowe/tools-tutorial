$.extend($.fn.datagrid.defaults, {
    ...
    onFetchRows: function (param, cb) {
        $.ajax({
            type: 'GET', //opts.method,
            url: './data.json', //opts.url,
            data: param,
            dataType: 'json',
...
$.extend($.fn.datagrid.defaults.finder, {
    getRow: function (target, p) {	// p can be row index or tr object
        ...
        if (opts.view.type == 'scrollview') {
            (index >= opts.view.index) && (index = index - opts.view.index);
        }
...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    renderRow: function (target, fields, frozen, rowIndex, rowData) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if (col) {
                ...
                if (col.checkbox) {
                    cc.push('<input type="checkbox"' + (value ? ' checked ' : '') + 'name="' + field + '" value="' + (value != undefined ? value : '') + '">');
                } else if (col.expander) {
    ...
    getRows: function (target, page, callback) {
        ...
        if (rows.length && (rows.length == opts.pageSize || index + rows.length == state.data.total)) {
            ...
            //callback.call(this, rows); return;
            ...
            //opts.onFetchRows.call(target, page, rows);
            var that = this;
            opts.onFetchRows.call(target, param, function (rows) {
                $(target).datagrid('loaded');
                callback.call(that, rows);
            });
        } else {
            var param = $.extend({}, opts.queryParams, {
                page: page,
    ...
    insertRow: function (target, index, row) {
        ...
        if (index > data.rows.length) {
            var opts = $(target).datagrid('options');
            curIndex = index - opts.view.index;
            //index = data.rows.length;
        }
        else {
            curIndex = index;
        }
        $.fn.datagrid.defaults.view.insertRow.call(this, target, curIndex, row);
        if (data.firstRows && index < data.firstRows.length) {
            data.firstRows.splice(index, 0, row);
        } else {
            data.firstRows.push(row);
        }
    },
...
$.extend($.fn.datagrid.methods, {
    getChecked: function (jq) {
        return jq.datagrid('getRows').filter(function (el) {
            return el.modCheck
        })
    },
